...
#include <stdlib.h>
#include "board_clipped.c"

void test_check_neighbours(void){
  Board test_board;
  test_board.width=4;
  test_board.height=4;
  ...
  */
  ...
  CU_ASSERT_EQUAL(check_neighbours(test_board,2,0),2);
  CU_ASSERT_EQUAL(check_neighbours(test_board,1,2),2);
  CU_ASSERT_EQUAL(check_neighbours(test_board,3,3),0);
...
int main (){
  ...
  CU_pSuite suite = CU_add_suite("Game_tests", NULL, NULL);
  CU_pTest t1 = CU_add_test(suite, "neighbours_check", test_check_neighbours);
